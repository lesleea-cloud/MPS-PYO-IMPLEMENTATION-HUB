...
| `acm-pm` field | Changed from `<input type="text">` to `<select>` — populated from `TEAM_CONFIG.pms` on every modal open |
| `acm-hri` field | Changed from `<input type="text">` to `<select>` — populated from `TEAM_CONFIG.implHR` (HR/SI Implementer category only) on every modal open |
| `openAddClientModal()` | Now populates `acm-pm`, `acm-hri`, and `acm-impl` dropdowns dynamically from `TEAM_CONFIG` before resetting values |
| `acm-impl` dropdown source | Changed from `TEAM_CONFIG.implementers` (all) to `TEAM_CONFIG.implPYO` (PYO/PY Starter Implementers only) |
| `acm-gov` field (new) | Hidden "Sprout Gov Implementer" dropdown (`#acm-gov-row`) — shown only when Availed Service contains "Gov" OR Sprout Gov add-on is ticked; sourced from `TEAM_CONFIG.implGov`; saved to `d.gov` on submit |
| `acmCheckGov()` (new) | Toggles visibility of `#acm-gov-row` based on service selection and Sprout Gov checkbox state |
| `acm-otk-impl` field (new) | Hidden "OTK Implementer" dropdown (`#acm-otk-row`) — shown only when Outsourced Timekeeping add-on is ticked; sourced from `TEAM_CONFIG.implOTK`; saved to `d.otkImpl` on submit |
| `acmCheckOtk()` (new) | Toggles visibility of `#acm-otk-row` based on Outsourced Timekeeping checkbox state |
| `stSaveTeam()` | Refreshes `acm-pm`, `acm-hri`, `acm-impl`, `acm-gov`, and `acm-otk-impl` dropdowns after team config is saved; each uses its specific category |
| Overview tab HR-I cell | Changed from free-text `<input>` to `<select class="cl-hri-sel">` — options sourced from `TEAM_CONFIG.implHR`, current value pre-selected |
...
| **System** | Administration | Implementer only |
| **System** | Settings | Admin, Implementer (hidden for Manager) |
...
## Section 26 — Settings Access Control: Manager Removed, Admin Owns Team Config ✅ Coded (June 18, 2026)

### What changed
Manager role no longer has access to the Settings tab. Admin is now the role responsible for all team configuration.

### Role access matrix for Settings

| Panel | Admin | Manager | Implementer |
|---|---|---|---|
| Settings tab (nav) | ✅ visible | ✗ hidden | ✅ visible |
| Data Management (`st-panel-data`) | ✅ | ✗ (no access) | ✅ |
| Add Client (`st-panel-addclient`) | ✅ | ✗ (no access) | ✅ |
| Team Configuration (`st-panel-team`) | ✅ **now admin** | ✗ (no access) | ✗ |
| GitHub Sync (`st-panel-github`) | ✅ | ✗ (no access) | ✗ |

### Code changes

| Location | Before | After |
|---|---|---|
| `ROLE_CONFIG.manager.hideNav` | `['impl','my-clients','issues']` | `['impl','my-clients','issues','settings']` |
| `renderSettings()` — `st-panel-team` visibility | `isMgr` | `isAdmin` |
| `renderSettings()` — `tcRenderTeam()` call | `if(isMgr)` | `if(isAdmin)` |
| `renderSettings()` — `st-panel-github` visibility | `isAdmin\|\|isMgr` | `isAdmin` |
| `renderSettings()` — GitHub config population | `if(isAdmin\|\|isMgr)` | `if(isAdmin)` |
...
## Section 27 — Add Client Modal: Team-Config Dropdowns + Conditional Implementer Fields ✅ Coded (June 18, 2026)

### What changed

The Add New Client modal now sources its assignee fields from live `TEAM_CONFIG` data instead of free-text inputs, and conditionally shows implementer fields based on selected service/add-ons.

#### Field changes in modal

| Field | Before | After |
|---|---|---|
| Project Manager | `<input type="text">` | `<select>` populated from `TEAM_CONFIG.pms` |
| HR-I | `<input type="text">` | `<select>` populated from `TEAM_CONFIG.implHR` only |
| Implementer | `<select>` from all implementers | `<select>` from `TEAM_CONFIG.implPYO` only |
| Sprout Gov Implementer | hidden / not present | Conditional row (`acm-gov-row`), shown when Availed Service contains "gov" OR Sprout Gov add-on is ticked |
| OTK Implementer | hidden / not present | Conditional row (`acm-otk-row`), shown when Outsourced Timekeeping add-on is ticked |

#### New HTML elements

```html
<select id="acm-pm">…</select>
<select id="acm-hri">…</select>
<div id="acm-gov-row" style="display:none;">
  <select id="acm-gov">…</select>
</div>
<div id="acm-otk-row" style="display:none;">
  <select id="acm-otk-impl">…</select>
</div>
```

#### New / changed JS functions
...
## Section 28 — Team Config: localStorage Persistence + PM Save Fix ✅ Coded (June 18, 2026)

### Problem

Team configuration (especially Project Managers) was not persisting across page reloads. Root causes:

1. `stSaveTeam()` only wrote to Supabase, not localStorage — if the Supabase upsert failed (e.g., `impl_*` columns missing from the table), no persistence occurred.
2. `loadFromSupabase()` unconditionally overwrote `TEAM_CONFIG.pms` with whatever Supabase returned (including stale/old values from a failed prior upsert), then re-saved that stale value to localStorage, wiping out locally-saved PM changes.

### Fix

Three-part fix:

#### 1. TEAM_CONFIG IIFE — `_fromLocalStorage` flag

When `TEAM_CONFIG` is initialised from `pyo_team` in localStorage, the returned object includes `_fromLocalStorage: true`. This flag signals that the user has explicitly saved team config locally and that Supabase should not override it.

```javascript
var TEAM_CONFIG = (function(){
  try{
    var s=localStorage.getItem('pyo_team');
    if(s){var t=JSON.parse(s);t.implementers=…;t._fromLocalStorage=true;return t;}
  }catch(e){}
  return {/* defaults */};
})();
```

#### 2. `loadFromSupabase()` — guard PM override

```javascript
// Before: if(rt.data.pms)TEAM_CONFIG.pms=rt.data.pms;
// After:
if(rt.data.pms && !TEAM_CONFIG._fromLocalStorage)TEAM_CONFIG.pms=rt.data.pms;
```

If localStorage already has team data, Supabase is not allowed to override `pms`. Supabase is only used as the source when no local backup exists (e.g., first login on a new device).

#### 3. `stSaveTeam()` — localStorage-first + PM-only Supabase fallback

```javascript
// Save to localStorage immediately (before Supabase)
try{localStorage.setItem('pyo_team', JSON.stringify({…pms:TEAM_CONFIG.pms}));}catch(e){}

// Try full upsert; on failure, retry with pms column only
_supa.from('team_config').upsert({id:1, impl_pyo:…, pms:…}, …).then(function(r){
  if(r.error){
    _supa.from('team_config').upsert({id:1, pms:TEAM_CONFIG.pms}, …).then(…);
  }
});
```

This ensures PMs are always saved to Supabase even if the `impl_*` columns haven't been added to the table yet.
