...
| **PM Tool "Fetch Now" returns 404** | index.html + api/integrations/save-pull-config.js | Root cause: user pasted the full path (`/api/external/projects`) into **both** the Base URL and Endpoint path fields under Settings → Integration → PM Tool pull settings. `pull.js` builds the request as `base_url + endpoint_path` with plain concatenation, so the path was duplicated (`.../api/external/projects/api/external/projects?...`), which the PM Tool's server 404'd. Added a guard — both client-side in `intgSavePullConfig()` and server-side in `save-pull-config.js` (`pathnameOf()` helper) — that rejects Save when the Endpoint path starts with the same path Base URL already ends with, with a message telling the user which field to trim. Does not auto-correct already-saved bad config; that must be fixed once in the UI (Base URL → origin only, Endpoint path → full path + query). (August 28, 2026) |
| **"Remove Key" on an integration fails with 502** | api/integrations/remove.js | `integrations.api_key` is `TEXT NOT NULL` (`PM_Tool_Integrations_Table.sql`), but `remove.js` PATCHed it to `null` — Postgres rejected the whole update, which the endpoint wraps as a generic 502. Changed to PATCH `api_key: ''` instead of `null`; `status` still flips to `'not_configured'`, which already gates the stale empty key from being used anywhere (`webhook.js` and `pull.js` both check `status !== 'configured'` before ever reading `api_key`). Also fixed `intgRemove()` and `intgSavePullConfig()` in index.html, which were discarding the server's actual error detail and showing only `"Server error <status>"` — both now surface the real `error` message from the response body, matching the pattern `intgPullNow()` already used. (August 28, 2026) |
| **Connecting/reconnecting a PM Tool key silently wipes Pull settings** | api/integrations/save.js | `intgConnect()` → `save.js` upserts via `POST /integrations` with `Prefer: resolution=merge-duplicates`, sending only `{id,tool_name,api_key,key_last4,status,configured_by,configured_at,updated_at}`. PostgREST's merge-duplicates upsert sets every column not present in the body to `NULL` on conflict — so `base_url`, `endpoint_path`, `auth_type`, `auth_header_name` (set separately by `save-pull-config.js`) got nulled out every time a key was (re)connected, even when the key value didn't change. Confirmed reproducible: after removing and reconnecting a fresh key, Base URL/Endpoint path/auth-type dropdown all reset to empty placeholders. Fixed by restricting the upsert to only the intended columns via PostgREST's `?columns=` query param, which scopes the `ON CONFLICT DO UPDATE SET` to just those fields and leaves the pull-config columns untouched. (August 28, 2026) |
...
## 68. Integrations — PM Tool pull, wired up (July 17, 2026)

The internal PM Tool's developer replied with real connection details: base URL `https://pmi.sprout.solutions`, endpoint `GET /api/external/projects?productContains=Payroll%20Outsourcing`, auth via a custom header `X-Api-Key`. Only the "Project Overview" scope was granted on that key, so the sample response has `projectId`, `lastUpdated`, `title`, `projectStatus`, `clientStage`, `segment`, `productsAvailed`, and `hasPayrollOutsourcing` populated — every other field is `null` until a broader scope is granted.

This closes the first "Not done yet" gap from §67: there's now a real outbound fetch, triggered manually.

- **API** — new `api/integrations/pull.js`. Loads the integration's stored `base_url`/`endpoint_path`/`auth_type`/`auth_header_name`/`api_key`, builds the request (query-param URLs for `bearer`/`custom_header`/`basic`/`query_param` per §67's four auth types), calls the tool's API with a plain `GET`, and expects either a top-level array or a `{ data: [...] }` shape back. Each record in the response is inserted as its own row into `integration_events` (`payload` = that one record) — the exact shape the push webhook (§49) already writes, so the existing PM Tool Data viewer (§57/§64) — filtering, suggested-schema box, assign/create/dismiss — needed zero changes to also handle pulled records.
- **UI** — added a "Fetch Now" button next to "Save Pull Settings" in the `#intg-pm_tool` pull-settings block. New JS `intgPullNow(id)` POSTs to `/api/integrations/pull`, toasts the number of records fetched, and calls the existing `refreshPmToolEvents()` if the PM Tool Data page happens to be open. The stale "isn't wired up yet" helper text below the block was replaced with a note that the credential used is whatever's pasted into the "Connect" key field above.
- **Key reuse, and what that means for §49's push path**: per §67's original design, the pull request's credential is the same `api_key` column the push webhook already validates incoming `Authorization: Bearer <key>` against (§49). The key the PM Tool's developer supplied (`sk-sprout-...dff98`) is *not* the same value as whatever was previously pasted into the "Connect" field (masked `••••••••8676` in earlier screenshots) — saving the new key overwrites the old one. Since no PM Tool push integration was ever actually connected on the other end (§64: "no real PM Tool is connected yet"), this has no live impact, but it means the push webhook path is now effectively retired in favor of pull, matching what §67 said the user wanted.

### Manual steps still required (not something code can do)
1. In Settings → Integrations → PM Tool, paste `sk-sprout-fc46062504741c7c63e059017c5dff98` into the key field and click Connect (this both authenticates future pulls and replaces whatever key was there before).
2. Fill in Pull settings: Base URL `https://pmi.sprout.solutions`, Endpoint path `/api/external/projects?productContains=Payroll%20Outsourcing`, auth method **Custom header**, header name `X-Api-Key`, then Save Pull Settings.
3. Click **Fetch Now** to run the first pull; check **PM Tool Data** for the staged records.
4. Redeploy to Vercel — this repo isn't git-linked (per §-level note in the sync-files memory), so pushing these file changes alone doesn't update the live site.

### Not done yet
- No scheduled/automatic pulling — `Fetch Now` is manual only, matching §67's "manual Sync now button" option (a schedule was the other option, not built).
- No field-mapping into Clients beyond what §64 already offers (manual assign, auto-preselect on name match) — still relevant only for the 7 populated fields until a broader API scope is granted.
- `basic` auth is implemented as `Base64(api_key)` with no separate username, since the schema only stores one secret value per integration — fine for `custom_header` (the type actually in use), untested against a real `basic`-auth tool.

## 69. MOM Generator — switched from regex keyword-matching to real AI summarization (July 22, 2026)

Tested the MOM Generator against a real simulation set (AspiraGlobal payroll discussion transcript + the expected MOM output for it) and found the extraction couldn't get close: `momExtract()` was 100% regex/keyword scoring — it split the transcript into lines, checked each against keyword lists (`agreedKw`/`clientKw`/`sproutKw`/`reminderKw`), and copied matching raw lines into confidence-scored buckets (HIGH/MED/LOW). It could not paraphrase or synthesize — e.g. the transcript's ~30-line rambling SSS-contribution-basis exchange needed to collapse into one clean decision sentence, which no keyword match can produce.

Replaced the whole extraction path with a real Claude API call, reusing the exact browser-side calling pattern already used by the Payroll Policy Extractor (`ppExtract()`, §-level tool predating this log): same endpoint (`api.anthropic.com/v1/messages`, `model:'claude-sonnet-4-6'`), same header shape, same shared `localStorage['sprout_ai_key']` so a key saved in either tool works in both.

- **New `momExtractWithAI(transcriptText, pdfText, clientName, meetingDate, slidesName, sproutStaff, apiKey)`** (replaces `momExtract`) — keeps all the deterministic logic as-is (next-phase/meeting-type regex detection, `shortName`/`initials`/password generation, deck name), but sends the transcript (+ optional slide text) to Claude with a prompt asking it to read the full call and write four lists of clean, professionally-phrased sentences (`agreedItems`, `clientItems`, `sproutItems`, `reminderItems`) — explicitly instructed not to copy raw transcript fragments. Parses Claude's JSON response the same tolerant way `ppExtract` does (`txt.match(/\{[\s\S]*\}/)` then `JSON.parse`). The password is still generated client-side and appended after the AI call — never asked of the AI, since it's a fixed company-convention string.
- **API key field** — added a `mom-apikey` input + `momSaveKey()` to the MOM Generator UI, mirroring `pp-apikey`/`ppSaveKey()`, both reading/writing the same `sprout_ai_key` localStorage key.
- **Review step simplified to one continuous editable draft** — removed the confidence-scored per-item review panel (`momRenderReviewPanel`, HIGH/MED/LOW badges, per-item accept/reject) entirely. `momGenerate()` now goes straight from "Generate" to `momRenderDraftPreview()`, which renders the AI's draft as one `<div>`-per-line `contenteditable` block the user can freely rewrite, reorder, or delete lines from — no bucketed UI.
- **Export now reads the live edited draft, not the original AI output** — `momCopyToClipboard()` and `momDownload()` (both the `docx.js` path via `momBuildDocxInner()` and the RTF fallback via `momDownloadRTF()`) now pull lines from the DOM via `momPreviewLines()` at the moment of export, so edits the user makes to the draft are respected. A new `momClassifyLines()` walks those live lines and classifies each as title / section header (📍 → purple, 👉 or 🔔 → orange) / numbered item / blank / plain text, re-deriving the numbered-list sequence per section rather than trusting the user's literal digits — so adding, deleting, or reordering bullets in the draft still exports with correct Word formatting and numbering.
- **Deleted** (no longer had callers): `momExtract`, `momCleanLine`, `momResolveSubject`, `momMapPdfSections`, `momRenderReviewPanel`, `momSaveItemEdit`, `momRemoveReviewItem`, `momAddManualItem`, `momBuildFromConfirmed`, `momConfirmAndBuild`, the `#mom-review-panel` markup, and the `MOM_STATE.reviewItems`/`MOM_STATE.result` fields.

### Manual steps still required (not something code can do)
1. Paste a Claude API key into the new MOM Generator key field (or reuse one already saved from the Payroll Policy Extractor) before generating a MOM.
2. Redeploy to Vercel — this repo isn't git-linked, so saving these file changes alone doesn't update the live site.
